...
const ImageEditor: React.FC = () => {
  ...
  const handleGenerate = async () => {
    if (!prompt || !uploadedImage) {
      setError('Harap unggah gambar dan masukkan prompt.');
      return;
    }
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-6">
      <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg space-y-6">
        <div>
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">1. Unggah Gambar</label>
          <input id="file-upload" type="file" accept="image/png, image/jpeg" onChange={handleFileChange} className="hidden" />
          {imagePreview ? (
            <div className="relative group">
              <img src={imagePreview} alt="Pratinjau Unggahan" className="w-full rounded-md shadow-inner bg-gray-100 dark:bg-gray-700" />
              <div className="absolute inset-0 bg-black/50 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                 <button onClick={clearImage} className="bg-white text-red-500 font-bold py-2 px-4 rounded-lg hover:bg-red-500 hover:text-white transition-colors">Hapus</button>
              </div>
            </div>
          ) : (
            <label htmlFor="file-upload" className="flex justify-center w-full h-48 px-4 transition bg-white dark:bg-gray-700 border-2 border-gray-300 dark:border-gray-600 border-dashed rounded-md appearance-none cursor-pointer hover:border-secondary focus:outline-none">
              <span className="flex items-center space-x-2">
                ...
                <span className="font-medium text-gray-600 dark:text-gray-400">Letakkan file untuk Melampirkan, atau <span className="text-secondary underline">jelajahi</span></span>
              </span>
            </label>
          )}
        </div>
        <div>
          <label htmlFor="edit-prompt" className="block text-sm font-medium text-gray-700 dark:text-gray-300">2. Deskripsikan Editan Anda</label>
          <textarea id="edit-prompt" value={prompt} onChange={(e) => setPrompt(e.target.value)} rows={4} className="mt-1 block w-full px-3 py-2 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-secondary focus:border-secondary sm:text-sm dark:text-white dark:placeholder-gray-400" placeholder="contoh: tambahkan kota futuristik di latar belakang" />
        </div>
      ...
      <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg flex items-center justify-center min-h-[400px]">
        {isLoading ? (
          <div className="text-center">
            ...
            <p className="text-gray-600 dark:text-gray-300">Membuat gambar Anda, harap tunggu...</p>
          </div>
        ) : error ? (
          ...
          <div className="text-center space-y-4">
             ...
             <a href={generatedImage} download="gambar-hasil-edit.png" className="inline-block bg-accent text-white font-bold py-2 px-6 rounded-lg hover:bg-accent/90 transition-colors">Unduh Gambar</a>
          </div>
        ) : (
          <div className="text-center text-gray-500 dark:text-gray-400">
            ...
            <p>Gambar editan Anda akan muncul di sini.</p>
          </div>
        )}
      </div>
    </div>
