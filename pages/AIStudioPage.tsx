          ...
          {imagePreview ? (
            <div className="relative group">
              ...
              <div className="absolute inset-0 bg-black/50 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                 <button onClick={clearImage} className="bg-white text-red-500 font-bold py-2 px-4 rounded-lg hover:bg-red-500 hover:text-white transition-colors">Remove</button>
              </div>
            </div>
          ) : (
            <label htmlFor="file-upload" className="flex justify-center w-full h-48 px-4 transition bg-white dark:bg-gray-700 border-2 border-gray-300 dark:border-gray-600 border-dashed rounded-md appearance-none cursor-pointer hover:border-secondary focus:outline-none">
              <span className="flex items-center space-x-2">
                <svg xmlns="http://www.w3.org/2000/svg" className="w-6 h-6 text-gray-600 dark:text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" /></svg>
                <span className="font-medium text-gray-600 dark:text-gray-400">Drop files to Attach, or <span className="text-secondary underline">browse</span></span>
              </span>
            </label>
          )}
        </div>
        <div>
          <label htmlFor="edit-prompt" className="block text-sm font-medium text-gray-700 dark:text-gray-300">2. Describe Your Edit</label>
          <textarea id="edit-prompt" value={prompt} onChange={(e) => setPrompt(e.target.value)} rows={4} className="mt-1 block w-full px-3 py-2 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-secondary focus:border-secondary sm:text-sm dark:text-white dark:placeholder-gray-400" placeholder="e.g., add a futuristic city in the background" />
        </div>
      ...
      <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg flex items-center justify-center min-h-[400px]">
        {isLoading ? (
          <div className="text-center">
            <svg className="animate-spin h-10 w-10 text-secondary mx-auto mb-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
          ...
          <div className="text-center space-y-4">
             ...
             <a href={`data:image/png;base64,${generatedImage}`} download="generated-image.png" className="inline-block bg-accent text-white font-bold py-2 px-6 rounded-lg hover:bg-accent/90 transition-colors">Download Image</a>
          </div>
        ) : (
...
const ImageGenerator: React.FC = () => {
    ...
    const [isLoading, setIsLoading] = useState(false);
    const [error, setError] = useState<string | null>(null);
    const [copyButtonText, setCopyButtonText] = useState('Copy as Data URI');
    ...
    const handleGenerate = async () => {
        ...
        setError(null);
        setGeneratedImage(null);
        setCopyButtonText('Copy as Data URI');
        ...
        try {
            ...
            if (response.generatedImages && response.generatedImages.length > 0) {
                setGeneratedImage(response.generatedImages[0].image.imageBytes);
            } else {
                 setError("The AI didn't return an image. Try refining your prompt.");
            }
        } catch (err) {
    ...
    const handleCopy = () => {
        ...
        navigator.clipboard.writeText(dataUri).then(() => {
            ...
            setTimeout(() => setCopyButtonText('Copy as Data URI'), 2000);
        }).catch(err => {
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-6">
            <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg space-y-6">
                <div>
                    <label htmlFor="generate-prompt" className="block text-sm font-medium text-gray-700 dark:text-gray-300">1. Describe the Image You Want</label>
                    <textarea id="generate-prompt" value={prompt} onChange={(e) => setPrompt(e.target.value)} rows={4} className="mt-1 block w-full px-3 py-2 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-secondary focus:border-secondary sm:text-sm dark:text-white dark:placeholder-gray-400" placeholder="e.g., A majestic, iridescent dragon on a snowy mountain peak at sunrise." />
                </div>
            ...
            <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg flex items-center justify-center min-h-[400px]">
                {isLoading ? (
                    <div className="text-center"><svg className="animate-spin h-10 w-10 text-secondary mx-auto mb-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg><p className="text-gray-600 dark:text-gray-300">Generating your image, please wait...</p></div>
                ) : error ? (
                    <div className="text-center text-red-600 dark:text-red-400 bg-red-100 dark:bg-red-900/50 p-4 rounded-md"><p className="font-bold">Error</p><p>{error}</p></div>
                ) : generatedImage ? (
                    <div className="text-center space-y-4">
                    ...
                    <div className="text-center text-gray-500 dark:text-gray-400">
                        <svg xmlns="http://www.w3.org/2000/svg" className="w-16 h-16 mx-auto mb-4 text-gray-300" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
                        <p>Your generated image for articles & gallery will appear here.</p>
                    </div>
                )}
            </div>
        </div>
        ...
        <div className="space-y-8">
            <section className="text-center">
                ...
                <p className="text-lg text-gray-600 dark:text-gray-300 max-w-2xl mx-auto">
                    Your creative hub for generating and editing images with the power of AI.
                </p>
            </section>
