...
const EditArticlePage: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!formData.title || !formData.excerpt || !formData.content || !formData.imageUrl) {
        setSaveMessage({ text: 'Error: Please fill all required fields.', type: 'error' });
        setIsSaving(false);
        return;
    }
    ...
    setTimeout(() => {
        if (success) {
            setSaveMessage({ text: 'Article updated successfully!', type: 'success' });
            // Optionally refetch data to confirm
            const updated = getArticleBySlug(slug!);
            if(updated) setFormData(updated);

        } else {
            setSaveMessage({ text: 'Error: Could not update article.', type: 'error' });
        }
        setIsSaving(false);
  ...
  if (!article) {
    return <div className="text-center p-8">Loading article editor...</div>;
  }
  ...
  return (
    <div className="max-w-4xl mx-auto bg-white dark:bg-gray-800 p-8 rounded-lg shadow-lg">
      <h1 className="text-3xl font-bold mb-6 text-primary dark:text-gray-100">Edit Article</h1>
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
            <label htmlFor="content" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Content (one paragraph per line)</label>
        ...
        <div>
            <label htmlFor="imageUrl" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Image URL or Data URI</label>
            ...
            <Link to="/ai-studio" target="_blank" rel="noopener noreferrer" className="text-sm text-secondary hover:underline mt-1 inline-block">Generate a new image with AI Studio &rarr;</Link>
        </div>
        <div>
        ...
        <div className="flex items-center justify-between pt-4 border-t dark:border-gray-700">
          <Link to="/admin" className="text-secondary hover:underline">&larr; Back to Admin Panel</Link>
