...
// Komponen baru untuk blok kode dengan tombol salin
const CodeBlock: React.FC<{ code: string }> = ({ code }) => {
    ...
    return (
        <div className="my-6 relative bg-app-subtle p-4 rounded-md border border-app-default">
  ...
  const renderContent = (content: string[]): React.ReactNode => {
    ...
    content.forEach((paragraph, index) => {
      ...
      if (paragraph.startsWith('JSON_PROMPT:')) {
        ...
        elements.push(
          <CodeBlock key={`json-prompt-${index}`} code={jsonString} />
        );
        return;
      }
